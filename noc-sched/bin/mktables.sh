...
function summary()
{
  ...
  ${EXPLAB}sus2sus -i ${RNDSUS} -i ${LNGSUS} -i ${CNFLSUS} -o - -s 'float(nodes)' -c max \
    side_length nodes links capacity_bound bisection_bound bound num_patterns cycles_rnd cycles_lng cycles_cnfl time_rnd time_lng time_cnfl
}
...
# cleanup
rm ${GREPTMP} ${BIGSUS} ${RNDSUS} ${LNGSUS} ${CNFLSUS}